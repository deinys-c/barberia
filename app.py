...
# Contraseña fija para el panel del barbero (la puedes cambiar)
...
NUMERO_PRUEBA_VENEZUELA = "+584241234567"  # Cámbialo por el tuyo
...
def cancelar_cita_por_sistema(cita_id):
    ...
    conn.close()
    if afectadas > 0:
        print(f"⏰ [SISTEMA] Cita ID {cita_id} cancelada automáticamente por fallo en notificación (20 min).")
...
# Se envía la contraseña en cada petición (vía header o body)
def verificar_barbero():
    # Se espera que el frontend envíe la contraseña en el header X-Password
...
def login_barbero():
    ...
    if password == BARBERO_PASSWORD:
        return jsonify({'mensaje': 'Login exitoso', 'autenticado': True, 'token': 'fake-token'})
